...
class MainWindow(QMainWindow):
    ...
    def _setup_home(self):
        ...
        self.psi_out = QLineEdit(); self.psi_out.setReadOnly(True)
        h2.addWidget(self.psi_out)
        layout.addLayout(h2)
        # BPM output
        h3 = QHBoxLayout()
        h3.addWidget(QLabel("Current BPM:"))
        self.bpm_out = QLineEdit(); self.bpm_out.setReadOnly(True)
    ...
    def _reset_home(self):
        self.res_input.clear(); self.psi_out.clear(); self.bpm_out.clear()
    ...
    def _update_plot(self):
        bpm = get_bpm(); self.plot_bpm.setText(str(bpm))
        self._plot_data.append(bpm)
        if len(self._plot_data)>20: self._plot_data.pop(0)
        self.figure.clear(); ax=self.figure.add_subplot(111)
        ax.plot(self._plot_data); ax.set_title("BPM Over Time")

...
class SerialReader(QWidget):
    ...
    def __init__(self):
        ...
        self.serial_thread = SerialReaderThread(
            self.comport, self.baudrate, self.data_signal)
    ...
    def init_ui(self):
        ...
        self.pressure_curve = self.plot_widget.plot(
            pen='r', name="Pressure (bar)")
        self.position_curve = self.plot_widget.plot(
            pen='b', name="Position (°)")
...
if __name__ == '__main__':
    ...
    data_signal = pyqtSignal(float, float, float)
    window = SerialReaderThread("COM4", 9600, pyqtSignal)
    window.show()
    sys.exit(app.exec())

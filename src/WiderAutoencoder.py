...
class WiderAutoencoder(nn.Module):
    def __init__(self, n_channels):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(n_channels, 128, kernel_size=3, stride=2, padding=1),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(128, n_channels, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.Sigmoid()
        )
...
if __name__ == "__main__":
    ...
    image = images[0]
    model = WiderAutoencoder(n_channels=4)
    output = model(image)

...
class InverseKinematicsUR5:
    ...
    def getTheta1(self):
        ...
        p05 = self.gd.dot(
            np.array([0, 0, -self.d[5], 1]))-np.array([0, 0, 0, 1])
    ...
    def getTheta6(self):
        ...
        for i in range(2):
            T1 = transformDHParameter(
                self.a[0], self.d[0], self.alpha[0], self.theta1[i])
    ...
    def getTheta23(self):
        ...
        for i in range(2):
            ...
            for j in range(2):
                T45 = transformDHParameter(
                    self.a[4], self.d[4], self.alpha[4], self.theta5[i, j])
                T56 = transformDHParameter(
                    self.a[5], self.d[5], self.alpha[5], self.theta6[i, j])
                T14 = T16.dot(invTransform(T45.dot(T56)))

                P13 = T14.dot(
                    np.array([0, -self.d[3], 0, 1]))-np.array([0, 0, 0, 1])
                L = P13.dot(P13.transpose()) - self.a[1]**2 - self.a[2]**2

                if abs(L / (2*self.a[1]*self.a[2])) > 1:
                    if self.debug:
                        print('L3 = ', L, ' denominator = ',
                              (2*self.a[1]*self.a[2]))
                    self.flags3[i, j, :] = self.getFlags(
                        L, 2*self.a[1]*self.a[2])
                    L = np.sign(L) * 2*self.a[1]*self.a[2]
                self.theta3[i, j, 0] = acos(L / (2*self.a[1]*self.a[2]))
                self.theta2[i, j, 0] = -atan2(P13[1], -P13[0]) + asin(
                    self.a[2]*sin(self.theta3[i, j, 0])/np.linalg.norm(P13))
                self.theta3[i, j, 1] = -self.theta3[i, j, 0]
                self.theta2[i, j, 1] = -atan2(P13[1], -P13[0]) + asin(
                    self.a[2]*sin(self.theta3[i, j, 1])/np.linalg.norm(P13))
        if self.debug:

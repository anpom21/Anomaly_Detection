...
class SimpleNet(torch.nn.Module):
    ...
    def test_train(self, train_data):
        ...
        if os.path.exists(ckpt_path):
            ...
            self.forward_modules["feature_aggregator"].load_state_dict(state_dict["feature_aggregator"])
            self.forward_modules["preprocessing"].load_state_dict(state_dict["preprocessing"])
            self.forward_modules["preadapt_aggregator"].load_state_dict(state_dict["preadapt_aggregator"])

        aggregator = {"scores": [], "segmentations": [], "features": []}
        scores, segmentations, features, labels_gt, masks_gt = self.predict(train_data)
    ...
    def test(self, test_data, bad_images=False):
        ...
        if bad_images:
            scores = np.array(scores)[np.array(labels_gt) == 1].tolist()
            segmentations = np.array(segmentations)[np.array(labels_gt) == 1].tolist()
            labels_gt = np.array(labels_gt)[np.array(labels_gt) == 1].tolist()
        # auroc_1, full_pixel_auroc_1, pro_1 = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
        # aggregator["scores"].append(scores)
        # aggregator["segmentations"].append(segmentations)
        # aggregator["features"].append(features)
        #
        ...
        # return auroc, full_pixel_auroc, scores
        return scores, segmentations, labels_gt
    ...
    def train(self, training_data, test_data):
        ...
        def update_state_dict(state_dict):
            ...
            state_dict["feature_aggregator"] = self.forward_modules["feature_aggregator"].state_dict()
            state_dict["preprocessing"] = self.forward_modules["preprocessing"].state_dict()
            state_dict["preadapt_aggregator"] = self.forward_modules["preadapt_aggregator"].state_dict()
